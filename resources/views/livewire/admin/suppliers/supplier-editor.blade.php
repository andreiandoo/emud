<form wire:submit="save" class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            ...
            <h1 class="mt-2 text-2xl font-semibold tracking-tight">{{ $supplier?->exists ? 'Edit supplier feed' : 'New supplier feed' }}</h1>
        ...
        @if($supplier?->settings['profile'] ?? null)
            <div class="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm text-emerald-900">
    ...
    <section class="grid gap-4 rounded-xl border bg-white p-5 md:grid-cols-2">
        <label class="space-y-1"><span class="text-sm font-medium">Name</span><input wire:model="name" ></label>
        <label class="space-y-1"><span class="text-sm font-medium">Code</span><input wire:model="code" ></label>
        <label class="space-y-1"><span class="text-sm font-medium">Protocol</span><select wire:model="protocol" >@foreach($protocols as $item)<option value="{{ $item->value }}">{{ $item->value }}</option>@endforeach</select></label>
        <label class="space-y-1"><span class="text-sm font-medium">Connector class</span><input wire:model="connectorClass"  placeholder="Leave blank for protocol default"></label>
        <label class="space-y-1 md:col-span-2"><span class="text-sm font-medium">Catalog / material-master path</span><input wire:model="catalogEndpoint"  placeholder="Remote path, URL or wildcard supplied by the vendor"></label>
        <label class="space-y-1"><span class="text-sm font-medium">Price path</span><input wire:model="priceEndpoint" ></label>
        <label class="space-y-1"><span class="text-sm font-medium">Stock path</span><input wire:model="stockEndpoint" ></label>
        <label class="space-y-1"><span class="text-sm font-medium">Default currency</span><input wire:model="defaultCurrency" maxlength="3" ></label>
        <label class="space-y-1"><span class="text-sm font-medium">Supplier timezone</span><input wire:model="timezone" ></label>
        <label class="space-y-1"><span class="text-sm font-medium">Priority</span><input wire:model="priority" type="number" min="0" max="65535" ></label>
    ...
    <section class="card-padded">
        <h2 class="font-semibold">Qualification &amp; pipeline</h2>
        ...
        <div class="mt-4 grid gap-4 md:grid-cols-3">
            <label class="space-y-1"><span class="text-sm">Supplier type</span><select wire:model="supplierType" >@foreach($supplierTypes as $type)<option value="{{ $type->value }}">{{ $type->label() }}</option>@endforeach</select></label>
            <label class="space-y-1"><span class="text-sm">Onboarding status</span><select wire:model="onboardingStatus" >@foreach($onboardingStatuses as $status)<option value="{{ $status->value }}">{{ $status->label() }}</option>@endforeach</select></label>
            <label class="space-y-1"><span class="text-sm">Strategic role</span><select wire:model="strategicRole" ><option value="">—</option>@foreach($strategicRoles as $role)<option value="{{ $role->value }}">{{ $role->label() }}</option>@endforeach</select></label>
            <label class="space-y-1"><span class="text-sm">Country code</span><input wire:model="countryCode" maxlength="2"  placeholder="RO"></label>
            <label class="space-y-1 md:col-span-2"><span class="text-sm">Website</span><input wire:model="website"  placeholder="https://"></label>
            <label class="space-y-1"><span class="text-sm">Qualification score (0–100)</span><input wire:model="qualificationScore" type="number" min="0" max="100" ></label>
            <label class="space-y-1"><span class="text-sm">Integration readiness (1–5)</span><input wire:model="readinessScore" type="number" min="1" max="5" ></label>
            <label class="space-y-1"><span class="text-sm">4×4 fit score (0–100)</span><input wire:model="offroadFitScore" type="number" min="0" max="100" ></label>
            <label class="space-y-1 md:col-span-3"><span class="text-sm">Next action</span><input wire:model="nextAction"  placeholder="What must happen before this supplier can move forward"></label>
    ...
    <section class="card-padded">
        <h2 class="font-semibold">Commercial terms</h2>
        ...
        <div class="mt-5 grid gap-4 md:grid-cols-3">
            <label class="space-y-1"><span class="text-sm">Dropship fee / order</span><input wire:model="dropshipFee"  placeholder="empty = unknown"></label>
            <label class="space-y-1"><span class="text-sm">Packaging / handling fee</span><input wire:model="packagingFee" ></label>
            <label class="space-y-1"><span class="text-sm">Terms currency</span><input wire:model="termsCurrency" maxlength="3"  placeholder="EUR"></label>
            <label class="space-y-1"><span class="text-sm">Minimum order value</span><input wire:model="minimumOrderValue"  placeholder="0 = no minimum"></label>
            <label class="space-y-1"><span class="text-sm">Free shipping threshold</span><input wire:model="freeShippingThreshold" ></label>
            <label class="space-y-1"><span class="text-sm">Payment terms (days)</span><input wire:model="paymentTermsDays" type="number" min="0" max="365" ></label>
            <label class="space-y-1"><span class="text-sm">Return window (days)</span><input wire:model="returnWindowDays" type="number" min="0" ></label>
            <label class="space-y-1"><span class="text-sm">Restocking fee (%)</span><input wire:model="restockingFeePercent" ></label>
            <label class="space-y-1"><span class="text-sm">Return freight paid by</span><select wire:model="returnFreightPayer" ><option value="">Necunoscut</option><option value="reseller">Revânzător</option><option value="supplier">Furnizor</option><option value="case_by_case">De la caz la caz</option><option value="unknown">Declarat necunoscut</option></select></label>
            <label class="space-y-1"><span class="text-sm">Allowed destinations</span><input wire:model="allowedCountries"  placeholder="Empty = unrestricted. e.g. RO, BG, HU"></label>
            <label class="space-y-1"><span class="text-sm">Excluded destinations</span><input wire:model="excludedCountries"  placeholder="e.g. UA, RS"></label>
            <label class="space-y-1"><span class="text-sm">MAP / advertised price policy</span><input wire:model="mapPolicy" ></label>
        </div>
    </section>
    ...
    <section class="card-padded">
        <h2 class="font-semibold">Data rights</h2>
        ...
        <div class="mt-4 grid gap-4 md:grid-cols-2">
            <label class="space-y-1"><span class="text-sm">Rights class</span><select wire:model="rightsClass" >@foreach($rightsClasses as $rights)<option value="{{ $rights->value }}">{{ $rights->value }}</option>@endforeach</select></label>
            <label class="space-y-1"><span class="text-sm">License / agreement name</span><input wire:model="licenseName" ></label>
            <label class="space-y-1 md:col-span-2"><span class="text-sm">License / documentation URL</span><input wire:model="licenseUrl" ></label>
        </div>
    ...
    <section class="card-padded">
        <h2 class="font-semibold">Technical catalog promotion</h2>
        ...
        <div class="mt-4 grid gap-3 md:grid-cols-2">
            <label class="flex items-start gap-3 rounded-lg border p-4">
                ...
                <span><strong class="block text-sm">Enable technical promotion</strong><span class="field-hint">Stages supplier technical fields through the canonical assertion/provenance pipeline after catalog matching.</span></span>
            </label>
    ...
    <section class="card-padded">
        <div><h2 class="font-semibold">Per-supplier schedules</h2><p class="mt-1 text-xs text-stone-500">Enabled rows override the legacy global cadence for that supplier and mode.</p></div>
        <div class="mt-4 grid gap-4 lg:grid-cols-3">
            @foreach(['catalog'=>'Catalog / material master','prices'=>'Prices','stock'=>'Stock / availability'] as $mode=>$label)
                <div class="rounded-lg border p-4">
                    ...
                    <label class="mt-3 block space-y-1"><span class="text-xs text-stone-500">Cron expression</span><input wire:model="schedules.{{ $mode }}.cron" class="font-mono text-sm"></label>
                    <label class="mt-3 block space-y-1"><span class="text-xs text-stone-500">Timezone</span><input wire:model="schedules.{{ $mode }}.timezone" class="text-sm"></label>
                </div>
            @endforeach
        </div>
    </section>
