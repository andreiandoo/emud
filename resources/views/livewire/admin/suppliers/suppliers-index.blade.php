<div>
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div><h1 class="text-2xl font-semibold tracking-tight">Furnizori</h1><p class="text-stone-500">Pipeline comercial, capabilități, drepturi și controlul feedurilor.</p></div>
        <a href="{{ route('admin.suppliers.create') }}" class="btn-primary">Adaugă furnizor</a>
    </div>
        ...
        @foreach($statuses as $pipelineStatus)
            ...
            <button wire:click="$set('status', '{{ $status === $pipelineStatus->value ? '' : $pipelineStatus->value }}')"
                @class([
                    ...
                    'opacity-50' => $count === 0,
                ])>{{ $pipelineStatus->label() }} <span class="font-semibold">{{ $count }}</span></button>
        @endforeach
    </div>

    <div class="mb-6 grid gap-3 rounded-xl border bg-white p-4 sm:grid-cols-2 lg:grid-cols-4">
        <label class="space-y-1"><span class="text-xs text-stone-500">Caută</span><input wire:model.live.debounce.300ms="search" class="text-sm" placeholder="Nume sau cod"></label>
        <label class="space-y-1"><span class="text-xs text-stone-500">Rol strategic</span><select wire:model.live="role" class="text-sm"><option value="">Toate</option>@foreach($roles as $strategicRole)<option value="{{ $strategicRole->value }}">{{ $strategicRole->label() }}</option>@endforeach</select></label>
        <label class="space-y-1"><span class="text-xs text-stone-500">Țară</span><select wire:model.live="country" class="text-sm"><option value="">Toate</option>@foreach($countries as $countryCode)<option value="{{ $countryCode }}">{{ $countryCode }}</option>@endforeach</select></label>
        <button type="button" wire:click="resetFilters" class="self-end text-sm hover:bg-stone-50">Resetează filtrele</button>
    </div>
    ...
    @forelse($configured as $supplier)
        <section class="card-padded">
            <div class="flex items-start justify-between gap-3">
                ...
                <button wire:click="toggle({{ $supplier->id }})" @class(['rounded-full px-3 py-1 text-xs font-bold','bg-emerald-100 text-emerald-900'=>$supplier->is_active,'bg-stone-200 text-stone-600'=>!$supplier->is_active])>{{ $supplier->is_active ? 'activ' : 'oprit' }}</button>
            </div>
            ...
            @if($supplier->syncSchedules->isNotEmpty())
                <div class="mt-4 flex flex-wrap gap-2">
                    @foreach($supplier->syncSchedules as $schedule)
                        <span @class(['rounded border px-2 py-1 font-mono text-[11px]','border-emerald-200 bg-emerald-50 text-emerald-900'=>$schedule->is_enabled,'border-stone-200 bg-stone-50 text-stone-500'=>!$schedule->is_enabled])>{{ $schedule->mode }}: {{ $schedule->cron_expression }} · {{ $schedule->timezone }}</span>
                    @endforeach
                </div>
            @endif
            <div class="mt-4 flex flex-wrap gap-2">
        ...
        <table class="w-full text-sm">
            ...
            @forelse($prospects as $supplier)
                <tr class="align-top">
                    ...
                    <td @class(['p-3 text-right font-bold','text-emerald-700'=>$supplier->qualification_score >= 85,'text-stone-700'=>$supplier->qualification_score >= 65 && $supplier->qualification_score < 85,'text-red-700'=>$supplier->qualification_score !== null && $supplier->qualification_score < 65])>{{ $supplier->qualification_score ?? '—' }}</td>
