<div class="space-y-6">
    <div class="flex flex-wrap items-end justify-between gap-4">
        <div><div class="text-sm text-stone-500">Sursă</div><h1 class="text-2xl font-bold">{{ $source->name }} · Raw records</h1></div>
    ...
    <div class="grid gap-3 rounded-xl bg-white p-4 shadow-sm md:grid-cols-4">
        <input wire:model.live.debounce.300ms="search" class="rounded-lg border px-3 py-2" placeholder="External ID / eroare...">
        <select wire:model.live="status" class="rounded-lg border px-3 py-2"><option value="">Toate statusurile</option>@foreach(['unprocessed','published','skipped','ambiguous','failed'] as $value)<option value="{{ $value }}">{{ $value }}</option>@endforeach</select>
        <select wire:model.live="type" class="rounded-lg border px-3 py-2"><option value="">Toate tipurile</option>@foreach($types as $value)<option value="{{ $value }}">{{ $value }}</option>@endforeach</select>
