<div class="space-y-6">
    <div class="flex flex-wrap items-end justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold">Calitate & acoperire catalog</h1>
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        @foreach([
            ...
            <div class="rounded-xl border border-stone-200 bg-white p-4">
                ...
                <div class="mt-1 text-2xl font-bold tabular-nums">{{ number_format($value, 0, ',', '.') }}</div>
            </div>
        @endforeach
    </section>
    ...
    <section class="grid gap-4 lg:grid-cols-2">
        <div class="rounded-xl border border-stone-200 bg-white p-5">
            <h2 class="font-bold">Acoperire fitment</h2>
            <div class="mt-4 space-y-4">
                <div>
                    ...
                    <div class="mt-2 h-2 overflow-hidden rounded bg-stone-100"><div class="h-full bg-lime-400" style="width: {{ min(100, $metrics['part_fitment_coverage']) }}%"></div></div>
                ...
                <div>
                    ...
                    <div class="mt-2 h-2 overflow-hidden rounded bg-stone-100"><div class="h-full bg-lime-400" style="width: {{ min(100, $metrics['vehicle_fitment_coverage']) }}%"></div></div>
        ...
        <div class="rounded-xl border border-stone-200 bg-white p-5">
            <h2 class="font-bold">Semnale de calitate</h2>
    ...
    <section class="rounded-xl border border-stone-200 bg-white p-5">
        <h2 class="font-bold">Backlog canonicalizare</h2>
        <div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            @foreach(['unprocessed' => 'Neprocesate', 'candidate' => 'Candidate', 'ambiguous' => 'Ambigue', 'failed' => 'Eșuate'] as $status => $label)
                <div class="rounded-lg bg-stone-50 p-3"><div class="text-xs uppercase tracking-wide text-stone-500">{{ $label }}</div><div class="mt-1 text-xl font-bold">{{ number_format($metrics['backlog_by_status'][$status] ?? 0, 0, ',', '.') }}</div></div>
            @endforeach
        </div>
    </section>

    <section class="overflow-hidden rounded-xl border border-stone-200 bg-white">
        <div class="border-b border-stone-100 p-5"><h2 class="font-bold">Sănătatea surselor</h2><p class="text-sm text-stone-500">Backlog-ul este calculat din înregistrări neprocesate, candidate, ambigue sau eșuate.</p></div>
