<form wire:submit="save" class="space-y-6">
    <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
            ...
            <h1 class="mt-2 text-2xl font-semibold tracking-tight">{{ $source?->exists ? 'Edit source' : 'New source' }}</h1>
            @if($source?->exists)
        ...
        @if($source?->exists)
            <div class="flex flex-wrap gap-2">
                ...
                <button type="button" wire:click="runConfiguredModes" class="btn-primary">Run configured modes</button>
            </div>
        @endif
    </div>

    @if(session('status'))<div class="rounded-lg border border-emerald-300 bg-emerald-50 p-3 text-sm text-emerald-900">{{ session('status') }}</div>@endif
    ...
    @if($connectionResult)
        <section @class(['rounded-xl border p-4','border-emerald-200 bg-emerald-50'=>$connectionResult['ok'],'border-red-200 bg-red-50'=>!$connectionResult['ok']])>
            <div class="flex items-center justify-between gap-4"><h2 class="font-semibold">Connection test: {{ $connectionResult['ok'] ? 'OK' : 'FAILED' }}</h2><span class="text-xs text-stone-500">{{ $connectionResult['tested_at'] }}</span></div>
    ...
    <section class="grid gap-4 rounded-xl border bg-white p-5 md:grid-cols-2">
        <label class="space-y-1"><span class="text-sm font-medium">Name</span><input wire:model="name" >@error('name')<small class="text-red-600">{{ $message }}</small>@enderror</label>
        <label class="space-y-1"><span class="text-sm font-medium">Code</span><input wire:model="code" >@error('code')<small class="text-red-600">{{ $message }}</small>@enderror</label>
        <label><span class="text-sm">Source type</span><input wire:model="sourceType" class="mt-1"></label>
        <label><span class="text-sm">Protocol</span><select wire:model="protocol" class="mt-1"><option>http</option><option>database</option><option>sftp</option><option>ftp</option><option>file</option></select></label>
        <label class="md:col-span-2"><span class="text-sm">Connector class</span><input wire:model="connectorClass" class="mt-1" placeholder="App\Catalog\Sources\Connectors\..."></label>
        <label class="md:col-span-2"><span class="text-sm">Canonicalizer class</span><input wire:model="canonicalizerClass" class="mt-1" placeholder="App\Catalog\Canonicalization\Parts\ManufacturerPartCanonicalizer"><span class="text-xs text-stone-500">Use the generic manufacturer canonicalizer for structured brand/MPN/OE/EAN/fitment feeds; use a source-specific class only when needed.</span></label>
        <label><span class="text-sm">Base URL</span><input wire:model="baseUrl" class="mt-1"></label>
        <label><span class="text-sm">Catalog endpoint</span><input wire:model="catalogEndpoint" class="mt-1"></label>
    </section>

    <section class="card-padded">
        <h2 class="mb-4 font-bold">Rights</h2>
        <div class="grid gap-4 md:grid-cols-2">
            <label><span class="text-sm">Rights class</span><select wire:model="rightsClass" class="mt-1">@foreach($rightsClasses as $rights)<option value="{{ $rights->value }}">{{ $rights->value }}</option>@endforeach</select></label>
            <label><span class="text-sm">License name</span><input wire:model="licenseName" class="mt-1"></label>
            <label class="md:col-span-2"><span class="text-sm">License URL</span><input wire:model="licenseUrl" class="mt-1"></label>
        </div>
    ...
    <section class="card-padded">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h2 class="font-semibold">Import schedules</h2><p class="text-sm text-stone-500">Each mode has its own cron and timezone. The Laravel scheduler dispatches due rows.</p></div><button type="button" wire:click="addSchedule" class="text-sm font-semibold">+ Add schedule</button></div>
        <div class="mt-4 space-y-3">
            @forelse($schedules as $index=>$schedule)
                <div wire:key="source-schedule-{{ $index }}" class="grid gap-3 rounded-lg border p-4 md:grid-cols-[1fr_1.5fr_1.2fr_auto_auto] md:items-start">
                    <label><span class="text-xs font-medium uppercase text-stone-500">Mode</span><input wire:model="schedules.{{ $index }}.mode" class="mt-1 text-sm" placeholder="catalog">@error("schedules.$index.mode")<small class="text-red-600">{{ $message }}</small>@enderror</label>
                    <label><span class="text-xs font-medium uppercase text-stone-500">Cron</span><input wire:model="schedules.{{ $index }}.cron_expression" class="mt-1 font-mono text-sm" placeholder="0 2 * * *">@error("schedules.$index.cron_expression")<small class="text-red-600">{{ $message }}</small>@enderror</label>
                    <label><span class="text-xs font-medium uppercase text-stone-500">Timezone</span><input wire:model="schedules.{{ $index }}.timezone" class="mt-1 text-sm" placeholder="Europe/Bucharest">@error("schedules.$index.timezone")<small class="text-red-600">{{ $message }}</small>@enderror</label>
    ...
    <button class="btn-primary">Save source & schedules</button>
</form>
