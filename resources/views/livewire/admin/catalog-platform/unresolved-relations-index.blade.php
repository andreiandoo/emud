<div class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold">Unresolved part relations</h1>
            <p class="mt-1 text-sm text-stone-500">Cross-references and supersessions waiting for a canonical target.</p>
        </div>
        <button wire:click="retryPending" class="rounded-lg bg-stone-900 px-4 py-2 text-sm font-semibold text-white">Retry up to 500 pending</button>
    </div>
    ...
    <section class="grid gap-3 rounded-xl border bg-white p-4 md:grid-cols-4">
        <label class="space-y-1 md:col-span-2">
            ...
            <input wire:model.live.debounce.300ms="q" class="w-full rounded border px-3 py-2" placeholder="e.g. 90915-YZZD2, MANN, OC 123">
        </label>
        <label class="space-y-1">
            <span class="text-xs font-medium text-stone-500">Relation type</span>
            <select wire:model.live="relationType" class="w-full rounded border px-3 py-2">
        ...
        <label class="space-y-1">
            ...
            <select wire:model.live="sourceId" class="w-full rounded border px-3 py-2">
                <option value="">All</option>
                ...
                <div class="grid gap-5 xl:grid-cols-[1.2fr_1fr_auto]">
                    <div>
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span class="rounded px-2 py-1 text-xs {{ $relation->status === 'pending' ? 'bg-amber-100 text-amber-900' : ($relation->status === 'resolved' ? 'bg-lime-100 text-lime-900' : 'bg-red-100 text-red-900') }}">{{ $relation->status }}</span>
                    ...
                    <div>
                        ...
                        @if($relation->resolvedTargetPart)
                            <div class="mt-3 rounded bg-lime-50 p-3 text-sm text-lime-950">
                    ...
                    <div class="min-w-64 space-y-2">
                        @if($relation->status === 'pending')
                            <button wire:click="retry({{ $relation->id }})" class="w-full rounded border px-3 py-2 text-sm">Retry automatic match</button>
                            <input wire:model="manualTargets.{{ $relation->id }}" class="w-full rounded border px-3 py-2 font-mono text-xs" placeholder="prt_<public_id> or numeric ID">
                            @error('manualTargets.'.$relation->id)<div class="text-xs text-red-600">{{ $message }}</div>@enderror
                            <textarea wire:model="reviewNotes.{{ $relation->id }}" rows="2" class="w-full rounded border p-2 text-xs" placeholder="Optional review note"></textarea>
                            <button wire:click="link({{ $relation->id }})" class="w-full rounded bg-lime-400 px-3 py-2 text-sm font-semibold text-stone-950">Link canonical target</button>
                        ...
                        @elseif($relation->status === 'rejected')
                            <textarea wire:model="reviewNotes.{{ $relation->id }}" rows="2" class="w-full rounded border p-2 text-xs" placeholder="Reason for reopening"></textarea>
                            <button wire:click="reopen({{ $relation->id }})" class="w-full rounded border px-3 py-2 text-sm">Reopen</button>
                        @endif
                    </div>
                </div>
